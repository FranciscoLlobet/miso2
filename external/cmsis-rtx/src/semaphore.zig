...
pub const osSemaphoreAttr_t = c.osSemaphoreAttr_t;
...
pub fn new(max_count: u32, initial_count: u32, attr: ?*const osSemaphoreAttr_t) @This() {
    return @This().create(c.osSemaphoreNew(max_count, initial_count, attr));
}

/// Get semaphore name
pub fn getName(self: *const @This()) ?[*:0]const u8 {
    return c.osSemaphoreGetName(self.id);
}

/// Acquire a semaphore token or timeout if no tokens are available
pub fn acquire(self: *const @This(), timeout: u32) osError!void {
    return osErrorMap(c.osSemaphoreAcquire(self.id, timeout));
}

/// Release a semaphore token up to the initial maximum count
pub fn release(self: *const @This()) osError!void {
    return osErrorMap(c.osSemaphoreRelease(self.id));
}

/// Get current semaphore token count
pub fn getCount(self: *const @This()) u32 {
    return c.osSemaphoreGetCount(self.id);
}

/// Delete the semaphore
pub fn delete(self: *const @This()) osError!void {
    return osErrorMap(c.osSemaphoreDelete(self.id));
}
